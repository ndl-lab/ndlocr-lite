...
上記のコマンドでエラーが発生し導入に失敗する場合、uvで使用されるPythonの既定のバージョンが依存パッケージに対応していない可能性があります。このことが原因であれば、対応しているPythonのバージョンの指定を上記のコマンドに含めて実行すると導入に成功します。なお、指定するバージョンはあらかじめuvにインストールしておく必要があります。
