...
- [ ] **T2-2a**: `ndlocr-lite-web/src/ort/modelCache.ts` を実装。
...
- [ ] **T2-5a**: `ndlocr-lite-web/scripts/verify-ort.ts` に CLI スクリプトを作り、Node + Playwright ではなく **ブラウザ手動確認ページ**でも良いので、サンプル画像を JS 側だけで前処理 → ORT 実行 → 生テンソル出力を JSON で確認。
