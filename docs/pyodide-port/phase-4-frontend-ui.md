...
- [ ] **T4-1d**: 既存 `vite.config.ts` に `@vitejs/plugin-react` と Vite Worker プラグイン設定を追加。COOP/COEP ヘッダ・WASM コピーは Phase 2 で設定済みのため変更不要。
- [ ] **T4-1e**: Zustand・Comlink・Pyodide を追加（onnxruntime-web と idb-keyval は Phase 2 でインストール済み）。
  ```sh
  pnpm add zustand comlink pyodide
  ```
