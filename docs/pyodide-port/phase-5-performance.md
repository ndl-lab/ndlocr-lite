...
> - **ESLint 未設定**: T4-1c をスキップしたため `pnpm lint` スクリプルが存在しない。Phase 5 開始前に追加しないと T6-2a の CI が壊れる。**Phase 5 の最初のタスクとして対応推奨。** → ✅ **Phase 5 で実装済み**
...
- [x] **T5-0a**: Phase 4 完了時点の数値を `docs/pyodide-port/benchmark.md` に記録。（2026-04-20 実装済み）
  - 初回ロード総容量 / 総秒数
...
- [x] **T5-2a**: `ort.env.wasm.simd = true`, `ort.env.wasm.numThreads = <n>` を COOP/COEP 下で検証。（2026-04-20 実装済み — `ortSession.ts` に明示的に `simd = true` を追加）
- [x] **T5-2b**: `executionProviders: ['webgpu']` を優先（対応ブラウザのみ）。WebGPU 非対応時は `wasm`。（Phase 2/3 で実装済み）
...
- [x] **T5-4c**: 画像が極端に大きい（>4k）場合の事前スケーリング。DEIM の入力は 1024 固定なので、それ以上は事前に 2048 長辺まで落としても精度に影響しにくいと期待。（2026-04-20 実装済み — `pyodide.worker.ts` で OffscreenCanvas を使い 2048 px 上限にスケーリング）
...
- [x] **T5-5c**: UI バンドルは Vite の `build.target: 'es2022'` + `manualChunks` でベンダー分離。（2026-04-20 実装済み — react/comlink/zustand/jszip/idb-keyval を別チャンクに分割）

### T5-6: Service Worker / PWA 化

- [x] **T5-6a**: `vite-plugin-pwa` で Service Worker を生成し、モデル・Pyodide・UI 全部を precache。（2026-04-20 実装済み — UI バンドル + ORT WASM + wheel を Workbox でプリキャッシュ。モデルファイルは既存 Cache Storage ロジックで管理）
- [ ] **T5-6b**: 「完全オフライン動作」を自動テスト（Playwright の `context.setOffline(true)` で 2 回目訪問を検証）。
- [x] **T5-6c**: インストール可能な PWA（マニフェスト・アイコン）にし、デスクトップ / モバイル ホーム画面へ追加できるようにする。（2026-04-20 実装済み — `vite.config.ts` に PWA マニフェスト設定、`public/icons/icon.svg` アイコン、`index.html` に theme-color メタタグ追加）

### T5-7: メモリ最適化

- [x] **T5-7a**: 大きな画像から切り出した行画像（`npimg`）を都度 `del` + `gc.collect()`。（2026-04-20 実装済み — `cascade.py` の各ステージ後・`pipeline.py` の認識前後で npimg を None に設定し gc.collect()）
- [x] **T5-7b**: ORT セッションは OCR 完了後に `release()`するオプション（低メモリモード）を追加。（2026-04-20 実装済み — `ortSession.ts` に `releaseSession()` / `releaseAllSessions()` を追加、Worker 経由で `releaseOrtSessions()` として公開、`OcrClient.releaseOrtSessions()` として利用可能）
- [x] **T5-7c**: `navigator.deviceMemory < 4` の端末では警告 + 縮小画像に fallback。（2026-04-20 実装済み — `App.tsx` に `LowMemoryWarning` コンポーネントを追加、初回のみ表示して localStorage に dismiss 状態を保存）
...
| タスク | 概要 |
|--------|------|
| ESLint 設定 | `eslint.config.js` + `pnpm lint` スクリプト追加 |
| T5-0a ベースライン記録 | `docs/pyodide-port/benchmark.md` 作成 |
| T5-2a SIMD 明示 | `ort.env.wasm.simd = true` を `ortSession.ts` に追加 |
| T5-4c 大画像プリスケール | Worker で 2048 px 上限に OffscreenCanvas でスケール |
| T5-5c manualChunks | Vite でベンダー 5 種を別チャンクに分離 |
| T5-6a/c PWA + アイコン | `vite-plugin-pwa` + SVG アイコン + PWA マニフェスト |
| T5-7a Python gc | `cascade.py` / `pipeline.py` で各ステージ後に npimg 解放 |
| T5-7b ORT release API | `releaseAllSessions()` → Worker → `OcrClient.releaseOrtSessions()` |
| T5-7c 低メモリ警告 | `deviceMemory < 4` で amber バナーを初回のみ表示 |
