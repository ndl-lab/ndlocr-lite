...
def _detach_pdf_page(page):
    """
    1ページだけのPDFへ書き出して再読込し、
    他ページとの間接参照共有を切り離す。
    """
    ...
    # bufferとreaderの寿命をページ処理中まで維持するため、
    # 両方を返す。
    return reader.pages[0], reader, buffer
def embed_text_layer_pdf(
    input_pdf: str,
    output_pdf: str,
    page_results: list,
    ...
    try:
        ...
        raise RuntimeError(
            "PDF text-layer output requires pypdf and reportlab. "
    ...
    if output_path.resolve() == input_path.resolve():
        raise ValueError(
            "Output PDF must be different from the input PDF."
        )

    pdfmetrics.registerFont(
        UnicodeCIDFont("HeiseiKakuGo-W5", isVertical=False)
    )
    pdfmetrics.registerFont(
        UnicodeCIDFont("HeiseiMin-W3", isVertical=True)
    )

    source_reader = PdfReader(str(input_path))

    if len(source_reader.pages) != len(page_results):
        raise ValueError(
            "PDF page count mismatch: "
            f"{len(source_reader.pages)} pages, "
            f"{len(page_results)} OCR results"
        )

    #
    # 1. 入力PDFを一度メモリ上へ再書き出しする。
    #
    # 問題のあるPDFでページ間共有されているContentsやリソースを、
    # pypdfが扱いやすい形へ正規化する。
    #
    normalized_buffer = BytesIO()
    normalized_writer = PdfWriter()

    for source_page in source_reader.pages:
        normalized_writer.add_page(source_page)

    normalized_writer.write(normalized_buffer)
    normalized_buffer.seek(0)

    normalized_reader = PdfReader(normalized_buffer)

    #
    # 2. 全ページ分のオーバーレイを1つのPDFとして生成する。
    #
    # ページごとにCanvas/PdfReaderを作らず、
    # ReportLabのページ境界を明確にする。
    #
    overlay_buffer = BytesIO()

    first_page = normalized_reader.pages[0]
    ...
    for page_index, (page, page_result) in enumerate(
        zip(normalized_reader.pages, page_results)
    ):
    ...
    if len(overlay_reader.pages) != len(normalized_reader.pages):
        raise RuntimeError(
            "Overlay PDF page count mismatch: "
            f"{len(overlay_reader.pages)} overlay pages, "
            f"{len(normalized_reader.pages)} source pages"
        )

    #
    # 3. ページごとに対応するオーバーレイをマージする。
    #
    # writer.add_page()の戻り値を使うことで、
    # reader側のページを直接変更しない。
    #
    ...
    for page_index in range(len(normalized_reader.pages)):
        detached_page, detached_reader, detached_buffer = _detach_pdf_page(
            normalized_reader.pages[page_index]
        )

        # write完了までReaderとBufferを生存させる。
        detached_objects.append(
            (detached_reader, detached_buffer)
        )

        output_page = output_writer.add_page(detached_page)

        output_page.merge_page(
            overlay_reader.pages[page_index],
            expand=False,
        )

    with output_path.open("wb") as wf:
        output_writer.write(wf)
