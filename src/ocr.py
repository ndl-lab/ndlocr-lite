...
def _escape_markdown_paragraph(text: str) -> str:
    ...
    # Keep the table Markdown generated by TableCenterNet separate; this helper
    # is only for ordinary OCR paragraphs.
...
def _normalize_table_markdown_entries(table_markdowns):
    """Normalize table Markdown without changing v1.0.10 table association.

    TableCenterNet inference enumerates ``BLOCK TYPE="表組"`` elements in XML
    document order and stores that 1-based value as ``table_index``.  Keep that
    index authoritative.  Bounding-box matching is retained only as a fallback
    for older/external callers that do not provide an index.
    """
    entries = []
...
def build_markdown_text_from_page(page_elem, table_markdowns=None) -> str:
    ...
    Display rules:
      ...
      * A ``BLOCK TYPE="表組"`` is inserted at its LINE@ORDER / detected-bbox
        position and replaced by the matching TableCenterNet Markdown table.
